...
(function initV14FormUX(){
  ...
  /* HTML compatibility: expose the dynamic field actions globally.
     index.html may call these from onclick attributes. */
  window.addCustomerEmail=function(){
    const wrap=form.querySelector('.atd-v14-repeat-wrap[data-field="email"]');
    if(!wrap) return;
    const add=wrap.parentElement.querySelector('.atd-v14-add');
    if(add) add.click();
  };

  window.addCustomerPhone=function(){
    const wrap=form.querySelector('.atd-v14-repeat-wrap[data-field="phone"]');
    if(!wrap) return;
    const add=wrap.parentElement.querySelector('.atd-v14-add');
    if(add) add.click();
  };
